...
def draw_misc():
    ...
    if game_over:
        pygame.draw.rect(screen, "#00e6fc", [265, 215, 370, 120], 0, 10)
        pygame.draw.rect(screen, "#0d1117", [270, 220, 360, 110], 0, 10)
        gameover_text = font.render("Game over! Space bar to restart!", True, "#d52b1e")
        screen.blit(gameover_text, (290, 265))
    if game_won:
        pygame.draw.rect(screen, "#00e6fc", [265, 215, 370, 120], 0, 10)
        pygame.draw.rect(screen, "#151515", [270, 220, 360, 110], 0, 10)
        gameover_text = font.render("Victory! Space bar to restart!", True, "#00ff00")
...
def get_targets(red_x, red_y, blue_x, blue_y, green_x, green_y, yellow_x, yellow_y):
    ...
    if powerup:
        ...
        if not yellow.dead:
            ...
            yellow_target = return_target
    return [red_target, blue_target, green_target, yellow_target]
...
if __name__ == "__main__":
    ...
    for i in range(1, 5):
        player_images.append(
            pygame.transform.scale(
                pygame.image.load(f"C:/Users/yazdan/PacMan/PacMan/img/{i}.png"),
                (45, 45),
            )
        )
    red_img = pygame.transform.scale(
        pygame.image.load(f"C:/Users/yazdan/PacMan/PacMan/img/red.png"), (45, 45)
    )
    green_img = pygame.transform.scale(
        pygame.image.load(f"C:/Users/yazdan/PacMan/PacMan/img/green.png"), (45, 45)
    )
    blue_img = pygame.transform.scale(
        pygame.image.load(f"C:/Users/yazdan/PacMan/PacMan/img/blue.png"), (45, 45)
    )
    yellow_img = pygame.transform.scale(
        pygame.image.load(f"C:/Users/yazdan/PacMan/PacMan/img/yellow.png"), (45, 45)
    )
    spooked_img = pygame.transform.scale(
        pygame.image.load(f"C:/Users/yazdan/PacMan/PacMan/img/powerup.png"), (45, 45)
    )
    dead_img = pygame.transform.scale(
        pygame.image.load(f"C:/Users/yazdan/PacMan/PacMan/img/dead.png"), (45, 45)
    )
    ...
    while run:
        ...
        player_circle = pygame.draw.circle(
            screen, "#0d1117", (center_x, center_y), 20, 2
        )
        draw_player()
        red = Ghost(
            red_x,
        ...
        draw_misc()
        targets = get_targets(
            red_x, red_y, blue_x, blue_y, green_x, green_y, yellow_x, yellow_y
        )
        ...
        if not powerup:
            if (
                (player_circle.colliderect(red.rect) and not red.dead)
                or (player_circle.colliderect(blue.rect) and not blue.dead)
                or (player_circle.colliderect(green.rect) and not green.dead)
                or (player_circle.colliderect(yellow.rect) and not yellow.dead)
            ):
                if lives > 0:
                    lives -= 1
                    startup_counter = 0
        ...
        if (
            powerup
            and player_circle.colliderect(red.rect)
            and eaten_ghost[0]
            and not red.dead
        ):
            if lives > 0:
        ...
        if (
            powerup
            and player_circle.colliderect(blue.rect)
            and eaten_ghost[1]
            and not blue.dead
        ):
            if lives > 0:
        ...
        if (
            powerup
            and player_circle.colliderect(green.rect)
            and eaten_ghost[2]
            and not green.dead
        ):
            if lives > 0:
        ...
        if (
            powerup
            and player_circle.colliderect(yellow.rect)
            and eaten_ghost[3]
            and not yellow.dead
        ):
            if lives > 0:
